"""Differentiable Gaussian models for fitting peaks."""
...
class WarpedGaussian2D(nn.Module):
    ...
    def __init__(
        ...
        warp: torch.tensor = (1.0,),
        warp_angle: torch.tensor = (0.0,),
    ):
        super().__init__()
    ...
    def forward(self, grid):
        ...
        u = (grid_x - center_x) * torch.cos(warp_angle) - (
            grid_y - center_y
        ) * torch.sin(warp_angle)
        v = (grid_x - center_x) * torch.sin(warp_angle) + (
            grid_y - center_y
        ) * torch.cos(warp_angle)

        warped_gaussian = amplitude * torch.exp(
            -((u - warp * v**2) ** 2 / (2 * sigma_x**2) + v**2 / (2 * sigma_y**2))
        )
...
class Gaussian2D(nn.Module):
    ...
    def forward(self, grid):
        ...
        """
        # Add batch dimension
        grid_x = einops.rearrange(
            grid[..., 1], "h w -> h w" + " 1" * self.amplitude.dim()
        )
        grid_y = einops.rearrange(
            grid[..., 0], "h w -> h w" + " 1" * self.amplitude.dim()
        )
...
class Gaussian3D(nn.Module):
    ...
    forward(grid)
        ...
        Expects grid in zyx order (grid[..., 0] is z, grid[..., 1] is y,
        grid[..., 2] is x).
    """
    ...
    def forward(self, grid):
        ...
        # Add batch dimension
        grid_x = einops.rearrange(
            grid[..., 2], "d h w -> d h w" + " 1" * self.amplitude.dim()
        )
        grid_y = einops.rearrange(
            grid[..., 1], "d h w -> d h w" + " 1" * self.amplitude.dim()
        )
        grid_z = einops.rearrange(
            grid[..., 0], "d h w -> d h w" + " 1" * self.amplitude.dim()
        )
