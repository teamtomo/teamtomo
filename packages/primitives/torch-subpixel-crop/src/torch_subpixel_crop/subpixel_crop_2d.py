"""Extract 2D subimages with subpixel precision."""
...
def _extract_patches_batched(
    ...
    center = dft_center((ph, pw), rfft=False, fftshifted=True, device=images.device)
    grid = coordinate_grid(
        image_shape=(ph, pw), center=center, device=images.device
    )  # (ph, pw, 2)
    ...
    grid_flat = einops.rearrange(grid, "n_pos batch ph pw yx -> batch n_pos ph pw yx")
    ...
    images_rep = einops.rearrange(images, "batch h w -> batch 1 1 h w")
    ...
    patches = einops.rearrange(
        patches,
        "batch 1 n_pos ph pw -> n_pos batch ph pw",
    )
