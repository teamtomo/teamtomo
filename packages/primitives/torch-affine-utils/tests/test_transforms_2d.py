...
def test_batching():
    ...
    for op in [T, S]:
        matrix = op(torch.tensor([0, 90]))
        assert matrix.shape == (3, 3)

    # 1D tensors with length greater than 2 should raise an error for T and S
    for op in [T, S]:
        try:
            op(torch.tensor([0, 90, 180]))
        except ValueError:
            pass
        else:
            raise AssertionError(
                f"{op.__name__} should raise an error for 1D tensors with length > 2"
            )
