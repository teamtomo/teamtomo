...
def test_firstborn_vacuum_leaves_plane_wave_unchanged():
    ...
    exit_wave = firstborn(potential, pixel_size=1.0, voltage=300.0)
...
def test_firstborn_matches_explicit_per_slice_loop():
    ...
    exit_wave = firstborn(potential, pixel_size=pixel_size, voltage=voltage)
...
def test_firstborn_explicit_full_n_slices_matches_default():
    ...
    torch.manual_seed(0)
    potential = torch.randn(1, 5, 8, 8, dtype=torch.complex64)
    default_wave = firstborn(potential, pixel_size=1.0, voltage=300.0)
    explicit_wave = firstborn(potential, pixel_size=1.0, voltage=300.0, n_slices=5)
...
def test_firstborn_grouped_differs_from_ungrouped():
    ...
    ungrouped = firstborn(potential, pixel_size=1.0, voltage=300.0)
    grouped = firstborn(potential, pixel_size=1.0, voltage=300.0, n_slices=3)
...
def test_firstborn_grouped_pairs_with_empty_neighbor_match_full_resolution():
    ...
    wave_16 = firstborn(potential, pixel_size=1.0, voltage=300.0, n_slices=16)
    wave_8 = firstborn(potential, pixel_size=1.0, voltage=300.0, n_slices=8)
...
def test_firstborn_rejects_invalid_n_slices(n_slices):
    ...
    with pytest.raises(ValueError):
        firstborn(potential, pixel_size=1.0, voltage=300.0, n_slices=n_slices)
