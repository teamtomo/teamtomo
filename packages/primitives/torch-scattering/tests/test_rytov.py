"""Tests for the high-level torch_scattering.rytov() wrapper."""
...
def test_rytov_vacuum_leaves_plane_wave_unchanged():
...
def test_rytov_matches_explicit_per_slice_loop():
    ...
    stack `rytov` builds internally. Agreement checks that the batched
    ...
    expected = torch.exp(scattered_sum)

    assert torch.allclose(exit_wave, expected, atol=1e-5)


def test_rytov_approximately_conserves_energy_for_weak_potential():
    """A weak real potential must leave the exit wave close to unit amplitude.

    Rytov is the exponentiated Born series; for sigma*dz*V << 1 per slice,
    the exponent is small and exp(exponent) has modulus close to 1
    (approximate unitarity), unlike for strong potentials where the
    approximation can break energy conservation.
    """
    torch.manual_seed(0)
...
def test_rytov_explicit_full_n_slices_matches_default():
...
def test_rytov_grouped_differs_from_ungrouped():
...
def test_rytov_grouped_pairs_with_empty_neighbor_match_full_resolution():
...
def test_rytov_rejects_invalid_n_slices(n_slices):
