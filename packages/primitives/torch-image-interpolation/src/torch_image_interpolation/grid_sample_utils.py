"""Helpers converting array coordinates to ``grid_sample`` coordinates."""
...
def array_to_grid_sample(
    ...
    shape = torch.as_tensor(array_shape, dtype=dtype, device=device)
    grid_sample_coordinates = (array_coordinates / (0.5 * shape - 0.5)) - 1
...
def grid_sample_to_array(
    ...
    grid_sample_coordinates: torch.Tensor
        `(..., d)` array of coordinates for `torch.nn.functional.grid_sample`.
    array_shape: Sequence[int]
    ...
    shape = torch.as_tensor(array_shape, dtype=dtype, device=device)
    shape = torch.flip(shape, dims=(-1,))
    array_coordinates = (grid_sample_coordinates + 1) * (0.5 * shape - 0.5)
