...
def test_get_phase_shifts_3d_full_fft():
    ...
    expected = torch.tensor(
        [
            [
                [1.0 + 0.0000e00j, -1.0 - 2.3850e-08j],
                [1.0 + 1.7485e-07j, -1.0 - 6.7553e-07j],
            ],
            [
                [-1.0 - 8.7423e-08j, 1.0 + 3.4969e-07j],
                [-1.0 - 2.3850e-08j, 1.0 + 4.7700e-08j],
            ],
        ]
    )
...
def test_get_phase_shifts_3d_rfft():
    ...
    expected = torch.tensor(
        [
            [
                [1.0 + 0.0000e00j, -1.0 + 2.3850e-08j],
                [1.0 + 1.7485e-07j, -1.0 - 1.5100e-07j],
            ],
            [
                [-1.0 - 8.7423e-08j, 1.0 + 3.0199e-07j],
                [-1.0 - 2.3850e-08j, 1.0 + 0.0000e00j],
            ],
        ]
    )
    assert torch.allclose(phase_shifts, expected)
