...
def fourier_rescale_3d(
    ...
    if target_spacing is not None:
        if source_spacing is None:
            raise ValueError(
                "source_spacing is required when target_spacing is specified"
            )
        if target_shape is not None:
            raise ValueError("Cannot specify both target_spacing and target_shape")

        # Normalize to tuples. Unpacking to exactly 3 names (rather than casting)
        # gives a real runtime check: this raises if normalize_spacing(ndim=3)
        # ever returns something other than 3 values.
        source_d, source_h, source_w = normalize_spacing(source_spacing, 3)
        source_spacing_3d: tuple[float, float, float] = (source_d, source_h, source_w)
        target_d, target_h, target_w = normalize_spacing(target_spacing, 3)
        target_spacing_3d: tuple[float, float, float] = (target_d, target_h, target_w)
        ...
        source_d_px, source_h_px, source_w_px = image.shape[-3:]
        new_d, new_h, new_w = calculate_target_shape_from_spacing(
            (source_d_px, source_h_px, source_w_px),
            source_spacing_3d,
            target_spacing_3d,
        )
        target_shape = (new_d, new_h, new_w)

    # Case 2: Shape-based rescaling
    elif target_shape is not None:
        # Set default source_spacing if not provided
        source_d, source_h, source_w = normalize_spacing(
            1.0 if source_spacing is None else source_spacing, 3
        )
        source_spacing_3d = (source_d, source_h, source_w)
    ...
    # Fourier pad/crop
    image_d, image_h, image_w = image.shape[-3:]
    dft = fourier_rescale_rfft_3d(
        dft=dft,
        image_shape=(image_d, image_h, image_w),
    ...
    new_spacing_d, new_spacing_h, new_spacing_w = calculate_new_spacing(
        source_spacing_3d, (image_d, image_h, image_w), new_shape
    )
...
def fourier_rescale_rfft_3d(
    ...
    if target_d != d:
        dft = fourier_rescale_dimension(
            dft, dim=-3, source_dim_length=d, target_dim_length=target_d, is_rfft=False
        )

    # Handle height dimension (regular FFT dimension)
    if target_h != h:
        dft = fourier_rescale_dimension(
            dft, dim=-2, source_dim_length=h, target_dim_length=target_h, is_rfft=False
        )

    # Handle width dimension (rfft dimension)
    if target_w != w:
        dft = fourier_rescale_dimension(
            dft, dim=-1, source_dim_length=w, target_dim_length=target_w, is_rfft=True
        )
